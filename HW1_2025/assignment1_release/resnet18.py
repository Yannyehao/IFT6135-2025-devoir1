...
class ResNet18(nn.Module):
    ...
    def visualize(self, logdir):
        ...
        weights = self.conv1.weight.data.clone()
        # normalize to [0,1]
        min_w = weights.min()
        max_w = weights.max()
        weights = (weights - min_w) / (max_w - min_w + 1e-5)
        # make grid, save image
        grid = vutils.make_grid(weights, nrow=8, padding=2, normalize=False)
        vutils.save_image(grid, os.path.join(logdir, 'conv1_kernels.png'))       
